...
original_resfileindex_file_path = os.path.join(
    file_directory_path, "resfileindex_original.txt")
...
def read_file():
    ...
    with open(original_resfileindex_file_path, "r") as resfile_original:
        with open(resfileindex_file_path, "w") as resfile:
            for i in resfile_original:
                resfile.write(i)
    with open(resfileindex_file_path, "r") as file:
        resFile = file.read().split('\n')
        for i in range(len(resFile)):
            if "model/ship" in resFile[i]:
                if "concord" in resFile[i] or "mordu" in resFile[i] or "ore" in resFile[i] or "sleeper" in resFile[i] or "soe" in resFile[i] or "upwell" in resFile[i]:
                    if "effect" not in resFile[i] and "strategiccruiser" not in resFile[i] and "wreck" not in resFile[i]:
                        ...
                        if ("gr2" in resFile[i] or "ar.dds" in resFile[i] or "no.dds" in resFile[i] or "pmdg.dds" in resFile[i]):
                            if ("mediumdetail" not in resFile[i] or "lowdetail" not in resFile[i]):
                                appendedResFile.append(resFile[i])
                        elif "effect" not in resFile[i] and "strategiccruiser" not in resFile[i] and "wreck" not in resFile[i]:
                            # Get medium detail models
                            if "mediumdetail" in resFile[i]:
                                appendedResFile.append(resFile[i])
...
def convert_all_gr2():
    ...
    # Delete all .gr2 files
    for i in gr2_files_list:
        file = os.path.join(res_file_path, i)
        if os.path.isfile(file):
            os.remove(file)
        else:
            print("File not found, could not delete", file)
